...
class NetworkOps(object):
    ...
    @staticmethod
    def fully_connected(in_tensor, layer_name, out_chan, trainable=True, bias_boost=1.0):
        with tf.variable_scope(layer_name):
            in_size = in_tensor.get_shape().as_list()
            ...
            out_tensor = tf.matmul(in_tensor, weights) + bias_boost * biases
            return out_tensor
